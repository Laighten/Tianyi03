 <template>
    ...
    <el-table :data="tableData" height="780" border style="width: 100%" :row-class-name="tableRowClassName">
      <el-table-column 
        prop="SourseFrom"
        label="敏感出处"
        width="100">
    ...
    methods: {
          getdata(){
              return axios.get("/data/api/detailInfo").then(result=>{
                  ...
                  this.tableData = result.data.filter(function(e){return e.SourseFrom==="敏感视频";})
          ...
          tableRowClassName({row, rowIndex}) {
            ...
            // if (row.SourseFrom === "敏感文字") {
            //   return 'warning-word';
            // } else if (row.SourseFrom === "敏感图片") {
            //   return 'warning-picture';
            // }else if(row.SourseFrom === "敏感视频"){
